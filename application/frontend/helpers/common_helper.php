<?php

function string_url_safe($string,$dash=true,$strlower=true) 
{
    //remove any '-' from the string they will be used as concatonater
    if ($dash)
        $str = str_replace('-', ' ', $string);
    else
        $str = $string;

    $str = remove_sign($str);

    // remove any duplicate whitespace, and ensure all characters are alphanumeric
    if ($dash)
        $str = preg_replace(array('/\s+/', '/[^A-Za-z0-9\-]/'), array('-', ''), $str);

    // lowercase and trim
    if ($strlower)
        $str = trim(strtolower($str), " \t\n\r\0\x0B\-");
    return $str;
}

function remove_sign($str) 
{
    $sign = array("à", "á", "ạ", "ả", "ã", "â", "ầ", "ấ", "ậ", "ẩ", "ẫ", "ă",
        "ằ", "ắ", "ặ", "ẳ", "ẵ",
        "è", "é", "ẹ", "ẻ", "ẽ", "ê", "ề", "ế", "ệ", "ể", "ễ",
        "ì", "í", "ị", "ỉ", "ĩ",
        "ò", "ó", "ọ", "ỏ", "õ", "ô", "ồ", "ố", "ộ", "ổ", "ỗ", "ơ"
        , "ờ", "ớ", "ợ", "ở", "ỡ",
        "ù", "ú", "ụ", "ủ", "ũ", "ư", "ừ", "ứ", "ự", "ử", "ữ",
        "ỳ", "ý", "ỵ", "ỷ", "ỹ",
        "đ",
        "À", "Á", "Ạ", "Ả", "Ã", "Â", "Ầ", "Ấ", "Ậ", "Ẩ", "Ẫ", "Ă"
        , "Ằ", "Ắ", "Ặ", "Ẳ", "Ẵ",
        "È", "É", "Ẹ", "Ẻ", "Ẽ", "Ê", "Ề", "Ế", "Ệ", "Ể", "Ễ",
        "Ì", "Í", "Ị", "Ỉ", "Ĩ",
        "Ò", "Ó", "Ọ", "Ỏ", "Õ", "Ô", "Ồ", "Ố", "Ộ", "Ổ", "Ỗ", "Ơ"
        , "Ờ", "Ớ", "Ợ", "Ở", "Ỡ",
        "Ù", "Ú", "Ụ", "Ủ", "Ũ", "Ư", "Ừ", "Ứ", "Ự", "Ử", "Ữ",
        "Ỳ", "Ý", "Ỵ", "Ỷ", "Ỹ",
        "Đ", "ê", "ù", "à");

    $unsign = array("a", "a", "a", "a", "a", "a", "a", "a", "a", "a", "a"
        , "a", "a", "a", "a", "a", "a",
        "e", "e", "e", "e", "e", "e", "e", "e", "e", "e", "e",
        "i", "i", "i", "i", "i",
        "o", "o", "o", "o", "o", "o", "o", "o", "o", "o", "o", "o"
        , "o", "o", "o", "o", "o",
        "u", "u", "u", "u", "u", "u", "u", "u", "u", "u", "u",
        "y", "y", "y", "y", "y",
        "d",
        "A", "A", "A", "A", "A", "A", "A", "A", "A", "A", "A", "A"
        , "A", "A", "A", "A", "A",
        "E", "E", "E", "E", "E", "E", "E", "E", "E", "E", "E",
        "I", "I", "I", "I", "I",
        "O", "O", "O", "O", "O", "O", "O", "O", "O", "O", "O", "O"
        , "O", "O", "O", "O", "O",
        "U", "U", "U", "U", "U", "U", "U", "U", "U", "U", "U",
        "Y", "Y", "Y", "Y", "Y",
        "D", "e", "u", "a");
    return str_replace($sign, $unsign, $str);
}

/**
 * URL render
 */
function news_url($categoryUrl, $articleUrl = null)
{
    if ($articleUrl) {
        return base_url('tin-tuc/' . $categoryUrl . '/' . $articleUrl);
    } else {
        return base_url('danh-muc/' . $categoryUrl);
    }
}
/**
 * Translation
 */
function __($key)
{
    $ci =& get_instance();
    $translated = $ci->lang->line($key);
    if (!$translated) {
        return $key;
    }
    return $translated;
}
function _e($key)
{
    echo __($key);
}

/**
 * Captcha
 */
function init_captcha()
{
    return [
        'word'          => '',
        'img_path'      => './public/captcha/',
        'img_url'       => base_url('public/captcha/'),
        'font_path'     => base_url('public/bds/fonts/texb.ttf'),
        'img_width'     => '150',
        'img_height'    => 38,
        'expiration'    => 7200,
        'word_length'   => 8,
        'font_size'     => 25,
        'img_id'        => 'Imageid',
        'pool'          => '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ',

        // White background and border, black text and red grid
        'colors'        => array(
            'background' => array(255, 255, 255),
            'border' => array(255, 40, 40),
            'text' => array(0, 0, 0),
            'grid' => array(255, 255, 255)
        )
    ];
}